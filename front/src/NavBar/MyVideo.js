...
import { Layout, PageHeader, Card, Button, Avatar, message } from "antd";
import { DeleteTwoTone, DeleteFilled, DeleteOutlined } from "@ant-design/icons";
    ...
    videoList.map((video, index) => {
      ...
      const deleteVideo = (e) => {
        if (window.confirm("정말 삭제하시겠습니까?")) {
          api.delete(`video/deleteVideo/${video_Id}`).then((res) => {
      ...
      return (
        <Card.Grid style={cardStyle} className="videoBox" hoverable key={index}>
          <img
            style={cardimageStyle}
            src={`http://localhost:3001/${thumbnail}`}
            alt="썸네일"
            onClick={(e) => navigate(`/video/${video_Id}`)}
          />
